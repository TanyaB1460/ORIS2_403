...
public class PathScan {
    ...
    public static List<Class<?>> find(String scannedPackage) {
        ...
        URL scannedUrl = Thread.currentThread().getContextClassLoader().getResource(scannedPath); //находим папку
        if (scannedUrl == null) {
            throw new IllegalArgumentException("Bad package " + scannedPackage);
        }
        File scannedDir = new File(scannedUrl.getFile()); //превращаем url в объект папка
